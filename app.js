...
$("#form-aluna").addEventListener("submit", async (e) => {
  ...
  const payload = {
    ...
    altura: $("#aluna-altura").value ? parseFloat($("#aluna-altura").value) : null,
...
async function cadastrarAlunaDeResposta(resposta) {
  ...
  const altura = numeroDoTexto(resposta.altura_atual);
...
function renderQuestionario(r) {
  ...
  $("#questionario-peso-resumo").textContent = [
    ...
    r.altura_atual ? `Altura: ${r.altura_atual} m` : "",
...
function preencherFormularioPlano() {
  ...
  $("#plano-altura").value = a.altura ? r0(a.altura * 100) : (ultimo?.altura_cm ?? "");
...
function renderMontagemRefeicoes() {
  ...
  cont.innerHTML = calc.refeicoes.map((ref, i) => {
    ...
    const resumoOpcoes = [0, 1].map(pos => {
      ...
      const dif = (a, b) => { const d = a - b; return Math.abs(d) < b * 0.08 ? "" : ` <span class="${d > 0 ? "acima" : "abaixo"}">(${d > 0 ? "+" : ""}${nBR(r1(d))})</span>`; };
      return `
        <div class="resumo-opcao">
          ...
          <span class="resumo-real">na conta real dá ${r0(real.kcal)} kcal · P ${nBR(r1(real.proteina))}g${dif(real.proteina, ref.proteina)} · C ${nBR(r1(real.carbo))}g${dif(real.carbo, ref.carbo)} · G ${nBR(r1(real.gordura))}g${dif(real.gordura, ref.gordura)}</span>
        </div>`;
    }).join("");
...
function montarDocumento() {
  ...
  const vegetais = (state.alimentos || []).filter(a => a.categoria === "vegetal");

  return { refeicoes, frutas, vegetais };
}
...
<style>
  ...
  <h3>Vegetais à vontade</h3>
  <p>${doc.vegetais.map(v => v.nome).join(", ")}.</p>
  <p class="nota">Salada de folhas verdes cruas sempre à vontade. Chá à vontade após todas as refeições.</p>

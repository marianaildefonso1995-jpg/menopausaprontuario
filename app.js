...
function renderSemVinculo(respostas, alunas, ignoradoSet) {
  ...
  const opcoesAlunas = alunas.map(a => `<option value="${a.id}">${a.nome}</option>`).join("");
  ...
  $("#lista-sem-vinculo").innerHTML = filtradas.map(r => `
    <div class="lembrete-card">
      ...
      <div class="lembrete-texto">${r.telefone_respondente || ""}${r.email_respondente ? " · " + r.email_respondente : ""}</div>
      <div class="lembrete-acoes">
...
async function importarQuestionarios(texto) {
  ...
  const registros = linhasDados.map(cols => {
    const get = (campo) => {
  ...
  if (registros.length === 0) { toast("Nenhuma resposta válida encontrada no arquivo.", true); return; }

  const { error } = await sb.from("respostas_questionario").upsert(registros, { onConflict: "chave_importacao" });
  if (error) { toast("Erro ao importar: " + error.message, true); return; }

  const vinculadas = registros.filter(r => r.aluna_id).length;
  const semVinculo = registros.length - vinculadas;
  toast(`Importado! ${vinculadas} vinculada(s) a alunas, ${semVinculo} sem vínculo.`);
  await carregarVisaoGeral();
}

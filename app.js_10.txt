...
function renderSemVinculo(respostas, alunas, ignoradoSet) {
  ...
  $all(".btn-cadastrar-nova").forEach(btn => btn.addEventListener("click", async () => {
    ...
    // tenta extrair o peso, altura e a meta de peso do questionário pra já preencher o cadastro
    ...
    // se veio peso no questionário, já lança o primeiro registro de pesagem
    if (pesoInicial != null) {
    ...
    toast(`"${resposta.nome_respondente}" cadastrada e vinculada!${pesoInicial != null ? " Peso inicial registrado." : ""}`);
    await carregarVisaoGeral();
  }));

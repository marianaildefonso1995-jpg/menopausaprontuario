...
// tipos de arquivo solto, que ficam na aba Arquivos
const TIPOS_ARQUIVO = [
  { valor: "plano", rotulo: "Plano alimentar" },
  { valor: "receita", rotulo: "Receita" },
  { valor: "pedido_exame", rotulo: "Pedido de exame" },
  { valor: "exame_resultado", rotulo: "Resultado de exame" },
  { valor: "entrega", rotulo: "Material entregue" },
  { valor: "outro", rotulo: "Outro" },
];

function rotuloTipo(categoria) {
  return TIPOS_ARQUIVO.find(t => t.valor === categoria)?.rotulo || "Outro";
}
...
function pedirArquivo(categoria, registroId, varios = false) {
...
// ------------------------------------------------------------
// ABA ARQUIVOS — PDFs soltos da aluna, sem precisar existir uma
// receita ou um plano cadastrado antes
// ------------------------------------------------------------
function garantirAbaArquivos() {
  if ($("#aba-arquivos")) return;

  const abas = document.querySelector(".abas");
  const painelEntregas = $("#aba-entregas");
  if (!abas || !painelEntregas) return;

  const botao = document.createElement("button");
  botao.className = "aba-botao";
  botao.dataset.aba = "arquivos";
  botao.textContent = "Arquivos";
  botao.addEventListener("click", () => mudarAba("arquivos"));
  const botaoQuestionario = abas.querySelector('.aba-botao[data-aba="questionario"]');
  if (botaoQuestionario) abas.insertBefore(botao, botaoQuestionario);
  else abas.appendChild(botao);

  const painel = document.createElement("div");
  painel.id = "aba-arquivos";
  painel.className = "aba-conteudo hidden";
  painel.innerHTML = `
    <p class="ajuda-campo">Guarde aqui o PDF do que você já mandou pra aluna — plano alimentar, receita, pedido de exame.
      Se ela perder, é só abrir e reenviar. Dá pra escolher vários arquivos de uma vez.</p>
    <div class="arquivo-envio">
      <label for="arquivo-tipo">Tipo do arquivo</label>
      <select id="arquivo-tipo">
        ${TIPOS_ARQUIVO.map(t => `<option value="${t.valor}">${t.rotulo}</option>`).join("")}
      </select>
      <button type="button" id="btn-subir-arquivo" class="btn-primario">📎 Escolher arquivo(s)</button>
    </div>
    <div id="lista-arquivos"></div>
  `;
  painelEntregas.insertAdjacentElement("afterend", painel);

  $("#btn-subir-arquivo").addEventListener("click", () => {
    pedirArquivo($("#arquivo-tipo").value, null, true);
  });
}

function renderArquivosDaAluna() {
  const cont = $("#lista-arquivos");
  if (!cont) return;

  const todos = Object.values(state.anexos || {}).flat();
  if (todos.length === 0) {
    cont.innerHTML = `<p class="vazio">Nenhum arquivo guardado dessa aluna ainda.</p>`;
    return;
  }

  // mais novos primeiro
  const ordenados = todos.slice().sort((a, b) => (b.created_at || "").localeCompare(a.created_at || ""));

  const porTipo = new Map();
  ordenados.forEach(anx => {
    const t = rotuloTipo(anx.categoria);
    (porTipo.get(t) || porTipo.set(t, []).get(t)).push(anx);
  });

  cont.innerHTML = Array.from(porTipo.entries()).map(([tipo, itens]) => `
    <div class="grupo-arquivos">
      <h4 class="grupo-arquivos-titulo">${tipo} <span class="grupo-arquivos-contagem">${itens.length}</span></h4>
      ${itens.map(anx => `
        <div class="linha-arquivo">
          <button type="button" class="anexo-abrir" data-id="${anx.id}" title="Abrir ${anx.nome}">
            ${extensaoDe(anx.nome) === "pdf" ? "📄" : "🖼️"} ${anx.nome}
          </button>
          <span class="linha-arquivo-meta">
            ${anx.created_at ? fmtData(anx.created_at.slice(0, 10)) : ""} · ${fmtTamanho(anx.tamanho)}${anx.registro_id ? " · preso a um registro" : ""}
          </span>
          <button type="button" class="btn-icone excluir anexo-excluir" data-id="${anx.id}" title="Excluir arquivo">🗑️</button>
        </div>`).join("")}
    </div>`).join("");
}
...
(function estiloEvolucao() {
  ...
  css.textContent = `
    ...
    .arquivo-envio label { margin: 0; }
    .arquivo-envio select { min-width: 180px; }
    .grupo-arquivos { margin-bottom: 18px; }
    .grupo-arquivos-titulo {
      margin: 0 0 6px;
      font-size: 13px;
    ...
    .linha-arquivo .anexo-abrir { max-width: 100%; font-size: 14px; }
    .linha-arquivo-meta { font-size: 12px; color: #7a7a83; flex: 1; }
    @media (max-width: 800px) {
      .arquivo-envio { flex-direction: column; align-items: stretch; }
      .arquivo-envio select, .arquivo-envio button { width: 100%; }
    }
  `;
